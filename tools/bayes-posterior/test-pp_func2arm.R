...
predp <- function(x, n, nmax, p0, a, b){
  ...
  for(i in 0:(nmax -n)){
    ...
    p2[i + 1] <- pbeta(p0, i + a + x, b + nmax - x - i, lower.tail = FALSE)
  }
...
#'
#' @examples
pprob = function(nmax = 40, nmin = 1, p0 = 0.3, p1 = 0.5, thetats = 0.8, 
...
#'   the first interim
#'
#' @return a plot of decision matrix
#' @export
#'
#' @examples
#' 
gngplot=function(pptabx, groupsize){
  ...
  plot(c(0, nc+2), c(0, nr+2), type="n", axes=F, xlab="", ylab="")
  text(c(1:nc),rep(nr+1, nc), cnames)
  text(rep(0, nr), 1:nr, rev(rnames))
  title(ylab="Number of Patients", line=0, cex.lab=1.2)
  text(nc/2, nr+2, "Number of Responses", cex=1.2)
  abline(h=c(0+0.5,nr+0.5))
  abline(v=c(0+0.5, nc+0.5))
  for (i in 1:nr) {
    j=nr-i+1
    x1=c(1:nc)[pptab[j,]==1]
    y=rep(i, length(x1))
    points(x1, y, pch=15, col="darkgreen", cex=3)
    x2=c(1:nc)[pptab[j,]==0]
    y=rep(i, length(x2))
    points(x2, y, pch=15, col="red", cex=3)
  }
}
...
#' @param pptab output from function \code{\link{pprob}}, the decision rule
...
plotsim <- function(psimx, p0, ptv,casen, lgdpos = 1){
  ...
  psim3 <- tidyr::pivot_longer(data = psim, cols = 2:4,
                               names_to = "Interim Decision", 
                               values_to = "prob") %>% 
...
tabsim = function(orr, nmin, nmax, nsim, pptab, groupsize=1){     ### orr can be an array
  ...
  ### pptab: outpur from function pprob(), is the decision rules
  npar=length(orr)
