...
## Method 
  (based on Natekin and Alois, 2013) Consider the data set $(\mathbf{x}, y)_{i = 1}^N$, where $\mathbb{x} = (x_1, \ldots, x_d)$ refers to the predictors, and $y$ to the corresponding labels of the response variable. The goal is to construc the unknown functional dependence $\mathbf{x}\overset{f}{\rightarrow}y$ with estimate $\hat{f}(\mathbf{x})$, such that some specific loss function $\Psi(y, f)$ is minimized
  \[\hat{f}(x) = y, \\ \hat{f}(x) = \arg\min_{f(x)}E_x[E_y[\Psi(y, f(x))]|x], \]
  
 where $E_y[\Psi(y, f(x))]$ is the expected loss and $E_x[~]$ is the expectation over all dataset.  We can restrict the function search space to a parametric family of functions $f(x, \theta)$, which will change the function optimization into parametric estimation 
